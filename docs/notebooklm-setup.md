...
- **공지 모달이 클릭을 가로채면 실패한다.** 2026-09-11 배치부터 새 노트북 위에 '5시간마다 한도 초기화'
  안내 모달이 떠서 `소스 추가 화면에 들어가지 못했다` 로 끝났다. `_dismiss_dialogs` 가 '대화상자 닫기' 로 닫는다.
  소스 추가 직전에만 닫으므로, 이후 단계(오디오 오버뷰·생성)에 새 모달이 뜨면 같은 방식으로 실패할 수 있다.
